...
import (
	...
	categoryService "complaint_server/internal/service/categoryService"
	complaintService "complaint_server/internal/service/complaintService"
...
func main() {
	...
	storage := setupStorage(ctx, cfg.ConnString, log)
	router := setupRouter(ctx, log, cfg, storage)

	startServer(cfg, router, log)
}
func setupStorage(ctx context.Context, connString string, log *slog.Logger) *pg.Storage {
	storage, err := pg.New(ctx, connString)
	if err != nil {
...
func setupRouter(ctx context.Context, log *slog.Logger, cfg *config.Config, storage *pg.Storage) chi.Router {
	...
	router.Use(cors.Handler(cors.Options{
		AllowedOrigins:   []string{"http://localhost:3000"}, // Разрешить фронту доступ
	...
	// Routes
	setupRoutes(ctx, cfg, router, log, storage)
	return router
}

func setupRoutes(ctx context.Context, cfg *config.Config, router chi.Router, log *slog.Logger, storage *pg.Storage) {
	complaintService := complaintService.New(storage)
	categoryService := categoryService.New(storage)
	...
	router.Route("/category", func(r chi.Router) {
		r.Get("/", categoriesGetAll.New(log, categoryService))                   //Удаление категории
	...
	router.Route("/admin", func(r chi.Router) {
		...
		r.Put("/complaint/{id}", update.New(log, complaintService))
		r.Delete("/complaint/{id}", deleteComplaint.New(log, complaintService)) //Удалить компл

		//Category
		r.Post("/category", categoriesCreate.New(log, categoryService))          //Создание категории
		r.Delete("/category/{id}", deleteCategoryById.New(log, categoryService)) //Удалить категории по АЙДИ
